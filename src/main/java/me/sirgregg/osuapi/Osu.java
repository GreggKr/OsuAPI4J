...
import me.sirgregg.osuapi.objects.Score;
...
public class Osu {
    ...
    public Score getScore(String beatmapId, int mode) {
        String url = baseURL + "get_scores?k=" + sanatizeURL(token) + "&b=" + sanatizeURL(beatmapId) + "&m=" + mode;

        String json = getJson(url);
        if (jsonExists(json)) {
            Gson gson = new GsonBuilder().create();
            return gson.fromJson(json, Score[].class)[0];
        }
